...
impl From<uuid::Error> for ApiError {
    fn from(error: uuid::Error) -> Self {
        error!("UUID parse from string error: {error:?}");

        Self::InternalError {
            message: "UUID Error".to_owned(),
        }
    }
}
...
///
/// ```rust
/// use auth2_api_shared::error::{ApiError, ApiErrorCode, ApiResult};
/// use auth2_api_shared::api_error;
///
/// #[macro_use]
...
/// # Example
/// ```
/// use auth2_api_shared::error::ApiError;
/// use auth2_api_shared::async_error;
/// use std::fmt;
///
/// #[macro_use]
/// extern crate tracing;
